...
vector<Vertex> loadUserGeneratedModel() {
	...
	for (auto i = 0u; i < faces; ++i) {
		...
		v0.position = (0, 0, 0);
		v1.position = Vec3f(radius * FW::cos(i * 2 * FW_PI / faces), -height, radius * FW::sin(2 * FW_PI / faces * (i + 1)));
		v2.position = Vec3f(radius * FW::cos(2 * FW_PI / faces * (i + 1)), -height, radius * FW::sin(2 * FW_PI / faces * (i + 2)));
		// Calculate the normal of the face from the positions and use it for all vertices.
		v0.normal = FW::cross(v2.position, v1.position).normalized();
		v1.normal = v0.normal;
		v2.normal = v0.normal;
		//
		// Some hints:
...
void App::render() {
	...
	modelToWorld.setCol(3, Vec4f(current_translation_[0],current_translation_[1],current_translation_[2],1));
